...
import dev.sapphic.smarthud.data.HotbarStyle;
import dev.sapphic.smarthud.event.InventoryCache;
...
public final class HotbarRender implements IRenderEvent {
    ...
    public void onRenderTickPre(RenderContext ctx) {
        ...
        int slots = cachedItems.size() < ModulesConfig.HOTBAR_HUD.slotLimit ? cachedItems.size() : ModulesConfig.HOTBAR_HUD.slotLimit;
        ...
        if (cachedItems.size() > 0) {
            ...
            for (int i = 0; i < slots; i++) {
                ...
                boolean renderOverlay = !stack.isStackable() && ModulesConfig.HOTBAR_HUD.renderOverlays;
                boolean showStackSize = cachedItem.getCount() > 1 && ModulesConfig.HOTBAR_HUD.showStackSize;
                ...
                if (showStackSize) {
                    int count = ModulesConfig.HOTBAR_HUD.mergeDuplicates ? cachedItem.getCount() : cachedItem.getActualCount();
    ...
    private int getAttackIndicatorOffset() {
        ...
        if (cachedItems.size() > 0) {
            int slots = cachedItems.size() < ModulesConfig.HOTBAR_HUD.slotLimit
                        ? cachedItems.size()
    ...
    private void renderHotbarBackground(RenderContext ctx, int x, int y, int slots) {
        int textureY = ModulesConfig.HOTBAR_HUD.hudStyle.getTextureY();
